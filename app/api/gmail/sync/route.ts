...
async function getValidToken(supabase: Awaited<ReturnType<typeof createServerSupabase>>, userId: string): Promise<string | null> {
...
async function listMessages(token: string, maxResults = 25): Promise<GmailMessageRef[]> {
  // Focus on recruiting-related messages in inbox
...
Each item must have:
{ "index": <number>, "emailType": "<type>", "company": "<name or empty>", "detectedAction": "<1 sentence action>", "detectedInterviewDate": "<YYYY-MM-DD or null>", "detectedDeadline": "<YYYY-MM-DD or null>" }
...
function domainCompany(email: string): string {
  ...
  if (!m) return ''
  const skip = new Set(['gmail', 'yahoo', 'hotmail', 'outlook', 'icloud', 'proton', 'mail', 'lever', 'greenhouse', 'workday', 'smartrecruiters', 'jobvite', 'myworkday', 'linkedin'])
...
export async function POST() {
  ...
  if (!token) {
    return NextResponse.json({ error: 'Gmail not connected or token expired. Please reconnect.' }, { status: 400 })
  }

  // Fetch message list
  const refs = await listMessages(token, 25)
  if (refs.length === 0) {
    await supabase.from('gmail_tokens').update({
  ...
  const messages = (
    await Promise.all(newRefs.slice(0, 20).map((r) => fetchMessage(token, r.id)))
  ).filter(Boolean) as GmailMessage[]

  // Build AI classification inputs
  const inputs: EmailInput[] = messages.map((msg, i) => {
    const body = extractBody(msg).slice(0, 1500)
    return {
      index: i,
